...
model = dict(
    ...
    criterion=dict(
        ...
        fix_mean_loss=True),
    fix_module=[],
    semantic_num_classes=semantic_num_classes, # 18 for SPFprmer
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            dict(type="RandomRotateT", angle=[-1 / 24, 1 / 24], axis="x", p=0.95),
            dict(type="RandomRotateT", angle=[-1 / 24, 1 / 24], axis="y", p=0.95),
            dict(type="RandomScaleT", scale=[0.8, 1.2]),
            dict(type="RandomTranslationT", shift=[0.5, 0.5, 0.5]),
            dict(type="CustomElasticDistortionT", distortion_params=[[20, 80], [60, 240]], p=0.95),
            dict(type="ChromaticAutoContrastT", p=0.5, blend_factor=None),
