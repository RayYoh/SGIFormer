...
from pointcept.custom.misc import split_offset
...
class SPFormerDecoder(nn.Module):
    ...
    def _get_query(self, input_dict):
        ...
        B = len(sp_feat)
        pe = self.pe.weight.unsqueeze(0).repeat(B, 1, 1) if \
            getattr(self, 'pe', None) else None
...
class SGIFormerDecoder(nn.Module):
    def __init__(
        ...
        self.rep_layer = nn.Sequential(
            ...
            nn.LayerNorm(num_sample_query), nn.ReLU()
        )
    ...
    def _get_query(self, input_dict, vx_logits=None, vx_bias=None):
        ...
        shift_coord = [_bias + _coord for _bias, _coord in zip(vx_bias, input_dict["vx_coord"])]
        sp_coord = [scatter_mean(_scoord[_inv], _sp, dim=0) 
        ...
        for i, (_score) in enumerate(vx_score):
            ...
            _, topk_idx = max_score.topk(int(self.alpha * _score.shape[0]), sorted=False)
