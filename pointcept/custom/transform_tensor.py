...
class RandomRotateT(object):
    ...
    def __call__(self, data_dict):
        ...
        angle = coord.new_tensor([np.random.uniform(self.angle[0], self.angle[1]) * np.pi])
        ...
        if "coord" in data_dict.keys():
            if self.center is None:
                ...
                center = coord.new_tensor([(x_min + x_max) / 2, (y_min + y_max) / 2, (z_min + z_max) / 2])
            else:
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == "all":
            ...
            if "index" not in data_dict.keys():
                data_dict["index"] = torch.arange(data_dict["coord"].shape[0], device=device)
            data_part_list = []
            ...
            if data_dict["coord"].shape[0] > point_max:
                ...
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    dist2 = torch.sum(
                        torch.pow(data_dict["coord"] - data_dict["coord"][init_idx], 2), 1
                    )
                    ...
                    delta = torch.square(
                        1 - data_crop_dict["weight"] / torch.max(data_crop_dict["weight"])
                    )
                ...
                data_crop_dict["weight"] = torch.zeros(data_dict["coord"].shape[0]).to(device)
        ...
        # mode is "random" or "center"
            ...
            if self.mode == "random":
                ...
                raise NotImplementedError
            idx_crop = torch.argsort(torch.sum(torch.square(data_dict["coord"] - center), 1))[
                :point_max
            ]
            if "coord" in data_dict.keys():
            ...
            if "superpoint" in data_dict.keys():
                data_dict["superpoint"] = torch.unique(data_dict["superpoint"][idx_crop], return_inverse=True)[1]
...
class CustomElasticDistortionT(object):
    ...
    def __call__(self, data_dict):
        if "coord" in data_dict.keys() and self.distortion_params is not None:
            ...
            data_dict["elastic_coord"] = torch.tensor(data_dict["elastic_coord"]).to(device)
        return data_dict
